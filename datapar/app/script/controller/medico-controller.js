'use strict'
app
        .controller('medicoCtrl', ['$state', '$scope', 'config', '$cookies', 'MedicoService', 'UsuarioService', function ($state, $scope, config, $cookies, MedicoService, UsuarioService) {
                $scope.formData = {};
                $scope.people = [];
...
//                            "X-Testing": "testing"
                ...
                $scope.modificar = function (personas) {
                    ...
                    $scope.personaModificar.nrocrm = personas.nrocrm;
                };
                $scope.usuario = function (personas) {
                    ...
                    $scope.personaUsuario.idmedico = personas.id;
                    UsuarioService.listarPorIdMedico(personas.id, cabecera).success(function (data) {
...
//                        swal("Mensaje del Sistema!", "No se pudo establecer conexión con el Servidor", "error");
                ...
                $scope.registrar = function () {
                    if ($scope.personaAgregar.nombre === "" || $scope.personaAgregar.nombre === undefined || $scope.personaAgregar.nombre === null ||
                        ...
                        MedicoService.crear($scope.personaAgregar, cabecera).success(function (data) {
                ...
                $scope.eliminar = function (persona) {
                    swal({
                        ...
                        closeOnConfirm: false},
                            function () {
                                MedicoService.eliminar(persona.id, cabecera).success(function (data) {
                ...
                $scope.actualizar = function () {
                    if ($scope.personaModificar.nombre === "" || $scope.personaModificar.nombre === undefined || $scope.personaModificar.nombre === null ||
                        ...
                        MedicoService.actualizar($scope.personaModificar, cabecera).success(function (data) {
                            if (data.nombre !== undefined || data.nombre !== "") {
